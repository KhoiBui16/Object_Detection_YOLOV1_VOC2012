...
def intersection_over_union(boxes_preds, boxes_labels, box_format="midpoint"):
    ...
    # .clamp(0) is the case when they do not intersect
...
def mean_average_precision(
    ...
    for c in range(num_classes):
        ...
        # Go through all predictions and targets, only add the ones that belongs to the current class c
        ...
        # Go through each (key, val) in dict
        for key, val in amount_bboxes.items():
            amount_bboxes[key] = torch.zeros(val)
            
        # sort by box probablities which is indes 2
        ...
        for detection_idx, detection in enumerate(detections):
            # Only take ground_truths that have same training idx as dectection
            ground_truth_img = [
